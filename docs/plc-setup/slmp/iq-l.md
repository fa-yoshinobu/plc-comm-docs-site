---
description: "PLC-side Ethernet settings for the MELSEC iQ-L built-in CPU port used with SLMP."
---

# MELSEC iQ-L — PLC-side settings

MELSEC iQ-L — built-in Ethernet port (CPU module).
...
| Keep-Alive | Enabled | UDP (survival confirmation) |

## Connecting with this library

| Parameter | Value |
|-----------|-------|
| Canonical profile | `melsec:iq-l` |
| Port (TCP) | `1025` |
| Port (UDP) | `1035` |
...
=== "Python"
    ...
    async def main() -> None:
        options = SlmpConnectionOptions(host="192.168.250.100", port=1025, plc_profile="melsec:iq-l")
...
=== ".NET (C#)"
    ...
    var options = new SlmpConnectionOptions("192.168.250.100", SlmpPlcProfile.IqL) { Port = 1025 };
...
![Unit parameters screen for the built-in Ethernet port](../images/slmp/iq-l/screenshot-01.png)
*Unit parameters screen for the built-in Ethernet port.*

![Remote device connection settings for SLMP communication](../images/slmp/iq-l/screenshot-02.png)
