---
description: "PLC-side settings for the MELSEC RJ71EN71 Ethernet communication unit used with SLMP."
---
...
Use the RJ71EN71 when the iQ-R CPU module does not have a built-in Ethernet port. The library profile is separate so configuration can name the communication unit explicitly. It uses 4E frames with iQ-R compatibility.
...
| Parameter | Value |
|-----------|-------|
| Canonical profile | `melsec:iq-r:rj71en71` |
| Frame and compatibility | 4E, iQ-R |
| Port (TCP) | `1025` |
| Port (UDP) | `1035` |
...
=== "Python"
    ...
    async def main() -> None:
        options = SlmpConnectionOptions(host="192.168.250.100", port=1025, plc_profile="melsec:iq-r:rj71en71")
...
=== ".NET (C#)"
    ...
    var options = new SlmpConnectionOptions("192.168.250.100", SlmpPlcProfile.IqRRj71En71) { Port = 1025 };
